...
1. Make sure the chanllenge_3 directory has these seven files (main.tf, provider.tf, nginx.sh.tpl, myproject.sh, versions.tf, tfversions_readme and Readme.txt)

  * Main.tf has the configuration of (s3(lifecycle policy), dynamodb, ec2, keypairs, security groups and their ports)
  * provider.tf has the configuration of the cloud provider (aws)
  *versions.tf file has the specific tool version to use for the project
...
#. now backend.tf file will be created in the challenge_3 directory in which the terraform remote backend configuration will be there.
#. Now it uses the aws profile (cloud_user) to login to the account, which is needed for re-initialization.

# now it will re-initialize Terraform with backend configuration.
4. Now it will ask you to enter YES or NO, if you enter YES the terraform.tf state file created in the local machine will be moved to remote s3 bucket backend.
...
5. The private_key with name pinky1.pem will stored in home/user/keys folder. The user can be any nmae on your system.
